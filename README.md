...
Inside an archived review, press `r` to toggle its comments on or off, `y` to copy the archived Markdown, or `q` to close it. The history list marks visible archives with `●` and hidden archives with `○`, and reopens after a toggle. `A` is disabled while viewing history; from the main review it archives every currently visible comment. Restore searches the saved code context, so comments follow line-number shifts when the selected code still exists. Comments whose context no longer exists are retained as unplaced warnings instead of being silently dropped.
